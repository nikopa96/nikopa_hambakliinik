    ...
    <div class="container">
      ...
      <div class="row">
        <div class="column col-12 col-md-5 pr-md-5">
          <h1 class="title">Visit booking</h1>
          <div class="mb-4">
            <label for="chooseProcedure">Select a procedure:</label>
            <select
              class="custom-select"
              id="chooseProcedure"
              v-model="chosenProcedure"
              @change="getVisits()"
            >
              <option
          ...
          <div class="mb-4">
            <label for="chooseDoctor">Select a doctor:</label>
            <select
              class="custom-select"
              id="chooseDoctor"
              ...
              @change="getVisits()"
            >
              <option selected v-bind:value="0">All doctors</option>
              <option
        ...
        <div
          class="column col-12 col-md-7 pl-md-5 d-none d-md-block"
          v-if="!chosenProcedure"
        >
          <img class="mt-md-5" :src="images.arrowLeft" alt />
          <p class="lead mt-4 text-center">
            Please select a procedure and a doctor
            <br />Possible visits will appear here
          </p>
        </div>
        <div class="column col-12 col-md-7 pl-lg-5 pl-0 pr-0 visits" v-else>
          <h1 class="title mb-3">Visits</h1>
          <nav class="nav mb-4 mb-4 ml-2 ml-md-0 sort">
            <div class="nav-link p-0 mr-3">Sort by:</div>
            <a
              class="nav-link p-0 mr-3"
              v-bind:class="{
                'sort__item--active': sorting === 'DOCTOR',
                'sort__item--inactive': sorting === 'TIME'
              }"
              v-on:click="getVisitsSortedByDoctor"
            >
              doctor
            </a>
            <a
              class="nav-link p-0 mr-3"
              v-bind:class="{
                'sort__item--active': sorting === 'TIME',
                'sort__item--inactive': sorting === 'DOCTOR'
              }"
              v-on:click="getVisitsSortedByTime"
            >
              time
            </a>
          </nav>
          <div v-if="sorting === 'DOCTOR'">
            <div
              v-for="visitByDoctor in visitsByDoctor"
              v-bind:key="visitByDoctor.doctorId"
            >
              ...
              <ul class="list-group mb-5 striped-list">
                <li
                  ...
                  v-for="visit in visitByDoctor.doctorVisits"
                  v-bind:key="visit.id"
                >
                  <h5 class="mb-0">{{ visit.time }}</h5>
                  <router-link
                    :to="{ name: 'register', params: { visitId: visit.id } }"
                  >
                    <a class="btn btn-book btn-md pl-4 pr-4" href role="button">
                      Booking
                    </a>
                  </router-link>
                </li>
          ...
          <div v-if="sorting === 'TIME'">
            <div
              v-for="visitByTime in visitsByTime"
              v-bind:key="visitByTime.interval"
            >
              <h4 class="visits__list__heading mb-4 ml-2 ml-md-0">
                {{ visitByTime.interval }}
              </h4>
              <ul class="list-group mb-5 striped-list">
                <li
                  class="list-group-item d-flex justify-content-between align-items-center pl-2 pr-2 pl-md-4 pr-md-4"
                  v-for="visit in visitByTime.doctorVisits"
                  v-bind:key="visit.id"
                >
                  <h5 class="mb-0 visits__list__doctor-time">
                    {{ visit.time }}
                  </h5>
                  <h5 class="mb-0 visits__list__doctor-name">
  ...
  data() {
    return {
      ...
      calendarSettings: {
        ...
        nextMonthCaption: { type: String, default: "Next" },
        prevMonthCaption: { type: String, default: "Previous" }
      }
    };
  },
  mounted() {
    ...
    const visitsApi = fetch("/dentalclinic/api/visits/getall").then(
      response => {
        return response.json();
      }
    );
      ...
      const results = this.visits.filter(visit => {
        if (this.chosenDoctor !== null && this.chosenDoctor !== 0) {
          return (
            ...
            visit.procedure.id === this.chosenProcedure &&
            visit.doctor.id === this.chosenDoctor
          );
        } else {
          return (
            visit.date === this.date &&
